...
Deploy redis-cart to a Kubernetes cluster. This Helm chart is autogenerated by `sconify-image`.
...
`image`|Workload image|`registry.scontain.com:5050/clenimar/test:redis-cart`
...
`nameOverride`|String to partially override redis-cart.fullname template with a string (will prepend the release name)|`nil`
`fullNameOverride`|String to fully override redis-cart.fullname template with a string|`nil`
...
`scone.configID`|SCONE config ID for the workload|`online-boutique-403614570/registry-scontain-com-5050-clenimar-test-redis-cart/redis-cart`
